...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4"><div><flux:heading size="xl">Roles</flux:heading><flux:text>Group permissions into reusable responsibilities.</flux:text></div>@can('create', App\Models\Role::class)<flux:button variant="primary" wire:click="create">Create role</flux:button>@endcan</div>
    @if(session('success'))<flux:callout variant="success">{{ session('success') }}</flux:callout>@endif
    <flux:input wire:model.live.debounce.300ms="search" label="Search roles" placeholder="Name or slug" class="max-w-md" />
    <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700"><table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">@forelse($roles as $role)<tr wire:key="role-{{ $role->id }}">
            <td class="px-4 py-4"><p class="font-medium">{{ $role->name }} @if($role->is_system)<flux:badge size="sm">System</flux:badge>@endif</p><p class="text-zinc-500">{{ $role->slug }}</p></td>
            <td class="px-4 py-4">{{ $role->permissions->count() }}</td><td class="px-4 py-4">{{ $role->users_count }}</td>
            <td class="px-4 py-4"><div class="flex gap-2">@can('update', $role)<flux:button size="sm" wire:click="edit({{ $role->id }})">Edit</flux:button>@endcan @can('delete', $role)<flux:button size="sm" variant="danger" wire:click="confirmDelete({{ $role->id }})">Delete</flux:button>@endcan</div></td>
    ...
    <flux:modal wire:model="showForm" class="w-full max-w-2xl">
        <form wire:submit="save" class="space-y-5"><flux:heading size="lg">{{ $editingId ? 'Edit role' : 'Create role' }}</flux:heading>
            ...
            <flux:input wire:model="slug" label="Slug" placeholder="record-manager" required />
            <flux:textarea wire:model="description" label="Description" />
            @can('roles.assign-permissions')
                <div class="space-y-4">@foreach($permissionGroups as $module => $permissions)<fieldset class="space-y-2 rounded-lg border border-zinc-200 p-4 dark:border-zinc-700"><legend class="px-2 font-medium">{{ ucfirst($module) }}</legend><div class="grid gap-2 sm:grid-cols-2">@foreach($permissions as $permission)@can($permission->slug)<flux:checkbox wire:model="selectedPermissions" :value="(string) $permission->id" :label="$permission->slug" />@endcan @endforeach</div></fieldset>@endforeach</div>
                <flux:error name="selectedPermissions" /><flux:error name="selectedPermissions.*" />
            @endcan
            <div class="flex justify-end gap-3"><flux:modal.close><flux:button>Cancel</flux:button></flux:modal.close><flux:button type="submit" variant="primary" wire:loading.attr="disabled">Save role</flux:button></div>
        </form>
    </flux:modal>
    <flux:modal wire:model="confirmingDelete" class="max-w-md">
        <form wire:submit="delete" class="space-y-5"><flux:heading size="lg">Delete role?</flux:heading><flux:text>{{ $assignedUsers }} user(s) will lose this role. Their accounts will remain. Type <strong>{{ $deletingSlug }}</strong> to confirm.</flux:text><flux:input wire:model="confirmation" label="Role slug confirmation" />
        <div class="flex justify-end gap-3"><flux:modal.close><flux:button>Cancel</flux:button></flux:modal.close><flux:button type="submit" variant="danger" wire:loading.attr="disabled">Delete role</flux:button></div></form>
    </flux:modal>
</div>
