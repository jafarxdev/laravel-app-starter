...
<div class="space-y-6">
    ...
    <div class="grid items-end gap-3 sm:grid-cols-3">
        <flux:input wire:model.live.debounce.300ms="search" label="Search permissions" placeholder="Name or slug" />
        <flux:select wire:model.live="moduleFilter" label="Module"><option value="">All modules</option>@foreach($modules as $moduleName)<option value="{{ $moduleName }}">{{ $moduleName }}</option>@endforeach</flux:select>
    ...
    <div class="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-700"><table class="w-full text-left text-sm">
        ...
        <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">@forelse($permissions as $permission)<tr wire:key="permission-{{ $permission->id }}">
            ...
            <td class="px-4 py-4"><div class="flex gap-2">@if($permission->isCritical())<flux:badge>Protected</flux:badge>@endif @can('update', $permission)<flux:button size="sm" wire:click="edit({{ $permission->id }})">Edit</flux:button>@endcan @can('delete', $permission)<flux:button size="sm" variant="danger" wire:click="confirmDelete({{ $permission->id }})">Delete</flux:button>@endcan</div></td>
    ...
    <flux:modal wire:model="showForm" class="w-full max-w-lg">
        <form wire:submit="save" class="space-y-5"><flux:heading size="lg">{{ $editingId ? 'Edit permission' : 'Create permission' }}</flux:heading>
            ...
            <flux:input wire:model="module" label="Module" placeholder="records" required />
            <flux:input wire:model="slug" label="Slug" placeholder="records.view" required />
            <flux:textarea wire:model="description" label="Description" />
            <div class="flex justify-end gap-3"><flux:modal.close><flux:button>Cancel</flux:button></flux:modal.close><flux:button type="submit" variant="primary" wire:loading.attr="disabled">Save permission</flux:button></div>
        </form>
    </flux:modal>
    <flux:modal wire:model="confirmingDelete" class="max-w-md"><div class="space-y-5"><flux:heading size="lg">Delete permission?</flux:heading><flux:text>This removes the permission from every role. Existing routes using it will deny access.</flux:text><div class="flex justify-end gap-3"><flux:modal.close><flux:button>Cancel</flux:button></flux:modal.close><flux:button variant="danger" wire:click="delete" wire:loading.attr="disabled">Delete permission</flux:button></div></div></flux:modal>
</div>
