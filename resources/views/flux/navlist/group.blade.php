...
>
    ...
    >
        ...
        <span class="text-sm font-medium leading-none">{{ __($heading) }}</span>
    </button>

    <div class="relative hidden space-y-[2px] ps-7 data-open:block" @if ($expanded === true) data-open @endif>
        <div class="absolute inset-y-[3px] left-0 ms-4 w-px bg-zinc-200 dark:bg-white/30"></div>
...
<div {{ $attributes->class('block space-y-[2px]') }}>
    <div class="px-1 py-2">
        <div class="text-xs leading-none text-zinc-400">{{ __($heading) }}</div>
    </div>
